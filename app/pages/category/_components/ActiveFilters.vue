...
<template>
  ...
  >
    ...
    <!-- Price -->
    <UBadge
      v-if="currentFilters.price.minPrice || currentFilters.price.maxPrice"
      color="neutral"
      variant="subtle"
    >
      Price: {{ priceLabel }}
    ...
    <!-- Attribute values -->
    <template
      v-for="filter in currentFilters.attributes"
      :key="filter.slug"
    >
      <UBadge
      ...
      <!-- Range values -->
      <UBadge
        v-if="filter.min !== undefined || filter.max !== undefined"
        color="neutral"
        variant="subtle"
      >
